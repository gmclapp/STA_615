...
gf_boxplot(HOME~PatientType:NurseAge,data=Example8.5.2,
       scales=list(x=list(rot=90))) + coord_flip()
```

Let's run a Levene's test here. We get a p-value of 0.2534 (if the levene test is run with respect to the median. Which is appropriate if the data are not normally distributed), so we don't reject the hypothesis of equal variances. We'll keep going.

```{r}
leveneTest(HOME ~ NurseAge*PatientType, data=Example8.5.2, center="mean")
#leveneTest(HOME ~ NurseAge*PatientType, data=Example8.5.2, center="median") This line would give the result in Dr. Zeitler's comment.
```
